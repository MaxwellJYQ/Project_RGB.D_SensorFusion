...
class DRSRDataset(Data.Dataset):
    def __init__(self, path, scale, dataset_name, RGB2Y=False, test_flag=False):
        self.scale = scale
        self.path = path
        self.test_flag = test_flag
        if test_flag:
            self.DepthHR_files = sorted(get_img_file(path+'/DepthHR/'))[8000:9000]
            self.RGB_files = sorted(get_img_file(path+'/RGB/'))[8000:9000]
            self.DepthLR_files = sorted(get_img_file(path+'/DepthLrUp/'))[8000:9000]
        else:
            self.DepthHR_files = sorted(get_img_file(path+'/DepthHR/'))[:8000]
            self.RGB_files = sorted(get_img_file(path+'/RGB/'))[:8000]
            self.DepthLR_files = sorted(get_img_file(path+'/DepthLrUp/'))[:8000]
        self.RGB2Y = RGB2Y
